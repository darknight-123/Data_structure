...
void BinarySearchTree<ItemType>::reverseLevelorder(void visit(BinaryNode<ItemType>&), BinaryNode<ItemType>* subtreePtr) const
{
	...
	do
	{
		...
		if (tmpPtr->getRightChildPtr() != nullptr)
		{
			nodeStack.push(tmpPtr->getRightChildPtr());
		}
		if (tmpPtr->getLeftChildPtr() != nullptr)
		{
			nodeStack.push(tmpPtr->getLeftChildPtr());
		}
		visit(*tmpPtr);
...
int BinarySearchTree<ItemType>::nonLeafCount_Recu(BinaryNode<ItemType>* subtreePtr)
{
	...
	{
		return 0 + nonLeafCount_Recu(subtreePtr->getRightChildPtr());
	}
	else if (subtreePtr->getRightChildPtr() == nullptr)

	{
		return 0 + nonLeafCount_Recu(subtreePtr->getLeftChildPtr());
	}
	// add your code here


	return 1+nonLeafCount_Recu(subtreePtr->getLeftChildPtr())+nonLeafCount_Recu(subtreePtr->getRightChildPtr());
}
#endif
